...
transformed parameters {
  ...
  for (i in 1:num_data) {
    Y_hat[i] = beta_0 + beta_covid * covid[i] + (beta_A + beta_inter_covid * covid[i]) * A[i] + dot_product(a_site[site[i]], B[:,i]);//B[:,i] will give you a vector that consists of all elements from the ith column of that matrix.
  }
}
