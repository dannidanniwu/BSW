#use Bayesian spline model without cluster specific intervention effect structure
#Fit data: with random effect of cluster specific intervention effect and cluster-specific time trend
library(simstudy)
library(data.table)
library(mgcv)
library(lme4)
...
s_model <- function(train_data, mod) {
  ...
  # Leave-One-Out Cross-Validation (LOO-CV) estimates
  loo_result <- fit$loo()
...
dir.create(file.path("/gpfs/data/troxellab/danniw/scratch/m2_cluster_specific_time_vary_trt_v2"), showWarnings = FALSE)

# Write the result to a file
output_filename <- paste0("output_iter_", iter, ".RData")
#save(result, file=output_filename)

save(result, file = paste0("/gpfs/data/troxellab/danniw/scratch/m2_cluster_specific_time_vary_trt_v2/", output_filename))
