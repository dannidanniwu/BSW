...
#mod <- cmdstan_model("./time_vary_cluster_specific_trt_our_m3_noloo.stan")
mod <- cmdstan_model("/gpfs/data/troxellab/danniw/r/BS/time_vary_cluster_specific_trt_our_m3_noloo.stan")
...
s_model <- function(train_data, mod) {
  ...
  knots_k <- quantile(train_data$k, probs=seq(0, 1, length=6)[-c(1, 6)])
  ...
  B_test_k <- predict(splines::bs(c(0: k_ex_mx), degree=3, knots=knots_k))[-1,]#-1:remove t=0
  
  
  stan_data <- list(num_data = nrow(train_data),
                    num_basis = ncol(B_train),
                    num_t_ex = nrow(B_test_t),
                    num_k_ex = nrow(B_test_k),
                    B = t(B_train),
                    B_t = t(B_t),
                    B_test_t=t(B_test_t),
                    B_test_k=t(B_test_k),
  ...
  bayes_time_eff = fit$summary(variables="time_eff",
                          posterior::default_summary_measures()[2])
  
  

  
  model_results <- data.table(true_tate,true_lte,div,num_max_tree_depth,t_ex_mx, k_ex_mx, t(bayes_phi[,2]),t(bayes_time_eff[,2]))
  colnames(model_results) <- c("true_tate","true_lte","div","num_max_tree_depth","t_ex_mx", "k_ex_mx",
                               t(bayes_phi[,1]),t(bayes_time_eff[,1]))
...
dir.create(file.path("/gpfs/data/troxellab/danniw/scratch/plot_m3_cluster_spf_time_vary_trt"), showWarnings = FALSE)

# Write the result to a file
output_filename <- paste0("output_iter_", iter, ".RData")

save(result, file = paste0("/gpfs/data/troxellab/danniw/scratch/plot_m3_cluster_spf_time_vary_trt/", output_filename))
