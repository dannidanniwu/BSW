...
s_model <- function(train_data, coefA, monot_mod) {
  set_cmdstan_path(path = "/gpfs/share/apps/cmdstan/2.25.0")
  ...
  knots_train <- quantile(train_data$k, probs=seq(0, 1, length=9)[-c(1, 9)])
  
  B_train <- predict(splines::bs(train_data$k, degree=3, knots=quantile(train_data$k, probs=seq(0, 1, length=9)[-c(1, 9)])))
...
s_single_rep <- function(iter, coefA, ncluster, list_of_defs, monot_mod ) {
  
  train_data <- s_generate(iter, coefA, ncluster, list_of_defs)
  
  model_results <- s_model(train_data, coefA, monot_mod )
  
  return(model_results)
}

s_replicate <- function(iter, coefA, ncluster, monot_mod) {
  list_of_defs = s_define()
  model_results = s_single_rep(iter, coefA, ncluster,list_of_defs, monot_mod)
  return(data.table(iter=iter, coefA = coefA , ncluster=ncluster, model_results))
}


scenarios = expand.grid(coefA= 5,ncluster=25)
i=1
nsim=100
...
save(res, file = paste0("/gpfs/data/troxellab/danniw/r/BS/", date_stamp, "/large_number_monot_time_vary_trt",coefA,"ncluster",ncluster,".rda"))
