...
mod <- cmdstan_model("/gpfs/home/dw2625/r/BS/stepped_wedge_time_vary_trt.stan")
...
s_model <- function(train_data, coefA, mod) {
  ...
  knots_t <- quantile(train_data$t, probs=seq(0, 1, length=9)[-c(1, 9)])
  
  B_t <- predict(splines::bs(train_data$t, degree=3, knots=quantile(train_data$t, probs=seq(0, 1, length=9)[-c(1, 9)])))
  #unique(B_t)
  ...
  model_results <- data.table(true_tate,true_lte,bayes_tate,bayes_lte,covered_bayes_tate,covered_bayes_lte,
                              gam_tate,gam_lte,div,num_max_tree_depth)
...
save(res, file = paste0("/gpfs/data/troxellab/danniw/r/BS/", date_stamp, "/large_number_m1_time_vary_trt",coefA,"ncluster",ncluster,".rda"))
