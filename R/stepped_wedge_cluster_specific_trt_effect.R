...
mod <- cmdstan_model("./stepped_wedge_time_spline_cluster_specific_treatment_effect.stan")
...
s_model <- function(train_data, test_data, mod) {
  ...
  B_train <- predict(bs(train_data$k, degree=3, knots=quantile(train_data$normk, probs=seq(0, 1, length=6)[-c(1, 6)])))
  ...
  B_test_tr <- predict(bs(test_data$k, degree=3, knots=knots_train))
