...
s_define <- function() {
  ...
  defOut <- defDataAdd(varname = "y", dist= "binary",formula = " a + b - 0.005 * k^2 + ..coefA * A", link="logit" )
...
s_model <- function(train_data, coefA) {
 # set_cmdstan_path(path = "/gpfs/share/apps/cmdstan/2.25.0")
  # #######Fitting the GAM model with default penalization 
  # fitsat <- glmer(y ~ A + as.factor(k) + (as.factor(k) | site), data = train_data, family = "binomial")
  ...
  # 
  # fitnotime <- glm(y ~ A, data = train_data, family = "binomial")
  ...
  # 
  # fitlntime <- glmer(y ~ A + k + (k | site), data = train_data, family = "binomial")
  ...
  fitgam <- mgcv::bam(y ~ A + s(k) + s(k, site, bs = "fs"), data = train_data, family = "binomial", method = "fREML")
  ...
  fitgam2 <- mgcv::bam(y ~ A + s(k) + s(site, A, bs = "re") + s(k, site, bs = "fs"), data = train_data, family = "binomial", method = "fREML") 
  ...
  model_results <- data.table(est_gam_freq= res_fitgam[1], se_gam_freq=res_fitgam[2], 
                              gam_lowci=range[1], gam_upci=range[2], 
  ...
  setnames(model_results, c("est_gam_freq", "se_gam_freq", "lowci_freq", "upci_freq", 
                            "est_gam_rdn_freq", "se_gam_rdn_freq", "lowci_freq_rdn", "upci_freq_rdn", "covered_freq", 
                            "covered_gam_rdn_freq"))
  
  model_results <- model_results%>%
    mutate(across(-c(covered_freq, covered_gam_rdn_freq), round, 3))
  
  return(model_results)
}

s_single_rep <- function(iter, coefA, ncluster, list_of_defs) {
  
  train_data <- s_generate(iter, coefA, ncluster, list_of_defs)
  
  model_results <- s_model(train_data, coefA)
  
  return(model_results)
}

s_replicate <- function(iter, coefA, ncluster) {
  list_of_defs = s_define()
  model_results = s_single_rep(iter, coefA, ncluster,list_of_defs)
...
scenarios = expand.grid(coefA=0,ncluster= 10)


i=1
coefA = scenarios[i,"coefA"]
ncluster= scenarios[i,"ncluster"]
# res <- replicate(50, s_replicate(iter=1,coefA = coefA,ncluster=ncluster
#                                     ))
...
sjob <- Slurm_lapply(1:160,
               ...
               job_name = "BS_118",
               sbatch_opt = list(time = "8:00:00",partition = "cpu_short", `mem-per-cpu` = "8G"),
...
save(res, file = paste0("/gpfs/data/troxellab/danniw/r/BS/", date_stamp, "/scenarios_trt0_",ncluster,".rda"))
